...
def ds_conv(in_c, out_c, kernel,stride=1,padding=1, bias=True,padding_mode='replicate'):
...
class PatchEmbed2D_dsconv(nn.Module):
    r""" Image to Patch Embedding
    Args:
        patch_size (int): Patch token size. Default: 4.
        in_chans (int): Number of input image channels. Default: 3.
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, **kwargs):
        ...
        if isinstance(patch_size, int):
            patch_size = (patch_size, patch_size)
        self.proj = ds_conv(in_chans, embed_dim, kernel=patch_size, stride=patch_size,padding=0)#nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
        if norm_layer is not None:
            self.norm = norm_layer(embed_dim)
        else:
            self.norm = None
...
class Final_PatchExpand2D(nn.Module):
    def __init__(self, dim, dim_scale=4, norm_layer=nn.LayerNorm):
        ...
        #        96             384
        self.expand = nn.Linear(self.dim, dim_scale * self.dim* dim_scale, bias=False)
        #                          24
...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs.view(B, K, -1, L), self.x_proj_weight.to(xs.device))
        dts, Bs, Cs = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=2)
        dts = torch.einsum("b k r l, k d r -> b k d l", dts.view(B, K, -1, L), self.dt_projs_weight.to(xs.device))
...
class VSSBlockv2(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.ln_1 = norm_layer(hidden_dim)  # 96             0.2                   16
        self.self_attention = SS2Dv2(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state)
        self.drop_path = DropPath(drop_path)
        self.mlp=gMlp(in_features=hidden_dim, hidden_features=2*hidden_dim, act_layer=nn.GELU, drop=0., channels_first=False)
...
class resi_VSSLayer(nn.Module):
    ...
    def forward(self, x):
        for idx,blk in enumerate(self.blocks):
            ...
            x=x+self.resi_convs[idx](x_tmp).permute(0,2,3,1)
